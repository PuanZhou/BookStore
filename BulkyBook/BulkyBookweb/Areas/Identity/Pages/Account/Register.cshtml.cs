...
namespace BulkyBookweb.Areas.Identity.Pages.Account
{
    public class RegisterModel : PageModel
    {
        ...
        private readonly IUnitOfWork _unitOfWork;//建立IUnitOfWork 透過DI注入
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    if (Input.Role == null)
                    {
                        await _userManager.AddToRoleAsync(user, SD.Role_User_Indi);//預設身分為個人用戶
                    }
                    else
                    {
                        await _userManager.AddToRoleAsync(user, Input.Role);//選取的身分
                    }
        ...
        private ApplicationUser CreateUser() //更改為ApplicationUser
        {
            try
            {
                return Activator.CreateInstance<ApplicationUser>(); //ApplicationUser
            }
            catch
            {
